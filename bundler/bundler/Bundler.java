...
public final class Bundler {
    private static final String GCS_JSON_UPDATER_VERSION = "1.0.0";
    ...
    private static final String YEARS                    = "2020";
    ...
    private static void checkPlatform() {
        ...
        if (osName.startsWith("Mac")) {
            OS = MACOS;
            PKG = Paths.get("GCSJsonUpdater-" + GCS_JSON_UPDATER_VERSION + ".dmg");
            NO_INSTALLER_PKG = Paths.get("GCSJsonUpdater.app");
        } else if (osName.startsWith("Win")) {
            OS = WINDOWS;
            PKG = Paths.get("GCSJsonUpdater-" + GCS_JSON_UPDATER_VERSION + ".msi");
            NO_INSTALLER_PKG = Paths.get("GCSJsonUpdater");
        } else if (osName.startsWith("Linux")) {
            OS = LINUX;
            PKG = Paths.get("gcs_json_updater-" + GCS_JSON_UPDATER_VERSION + "-1_amd64.deb");
            NO_INSTALLER_PKG = Paths.get("gcs_json_updater");
        } else {
    ...
    private static void createModules() {
        ...
        args.add("--create");
        args.add("--file");
        args.add(MODULE_DIR.resolve("com.trollworks.gcs-" + GCS_JSON_UPDATER_VERSION + ".jar").toString());
        args.add("--module-version");
        args.add(GCS_JSON_UPDATER_VERSION);
        args.add("--manifest");
        args.add(MANIFEST.toString());
        args.add("--main-class");
        args.add("com.trollworks.gcs.GCSJsonUpdater");
        args.add("-C");
    ...
    private static void createManifest() {
        try (PrintWriter out = new PrintWriter(Files.newBufferedWriter(MANIFEST))) {
            ...
            out.println("bundle-name: GCSJsonUpdater");
            out.println("bundle-version: " + GCS_JSON_UPDATER_VERSION);
    ...
    private static void packageApp(boolean noInstaller, boolean sign) {
        ...
        args.add("jpackage");
        args.add("--module");
        args.add("com.trollworks.gcs/com.trollworks.gcs.GCSJsonUpdater");
        ...
        args.add("Updates data files created by GCS prior to version 4.20 to the 4.20 format.");
        if (!noInstaller) {
        ...
        case WINDOWS -> {
            ...
            if (!noInstaller) {
                ...
                args.add("e1da7309-d492-418c-b5ed-0619717319af");
            }
        }
        }
        runNoOutputCmd(args);
