...
public class Technique extends Skill {
    ...
    public Technique(DataFile dataFile) {
        ...
        mDefault = new SkillDefault(SkillDefaultType.Skill, "Skill", null, 0);
        updateLevel(false);
    }
    ...
    public boolean satisfied(StringBuilder builder, String prefix) {
        if (mDefault.getType().isSkillBased()) {
            ...
            if (!satisfied && builder != null) {
                if (skill == null) {
                    builder.append(MessageFormat.format("{0}Requires a skill named {1}\n", prefix, mDefault.getFullName()));
                } else {
                    builder.append(MessageFormat.format("{0}Requires at least 1 point in the skill named {1}\n", prefix, mDefault.getFullName()));
                }
            }
            return satisfied;
        }
